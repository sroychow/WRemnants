...
template <std::size_t nEta, std::size_t nPt, std::size_t ch, std::size_t vars, std::size_t nhelicity>
class muon_eff_helper_stat_helicity {  
 public:
  using eff_t = Eigen::TensorFixedSize<double, Eigen::Sizes<nEta, nPt, ch, vars>>;
  using hel_t = Eigen::TensorFixedSize<double, Eigen::Sizes<nhelicity>>;
  using value_t = Eigen::TensorFixedSize<double, Eigen::Sizes<nhelicity, nEta, nPt, ch, vars>>;
  
  muon_eff_helper_stat_helicity() {}
  
  value_t operator() (const eff_t& et, const hel_t& ht) {
    constexpr std::array<Eigen::Index, 5> broadcastEff = { 1, nEta, nPt, ch, vars };
