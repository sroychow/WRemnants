...
def add_muon_efficiency_unc_hists(results, df, helper_stat, helper_syst, axes, cols, base_name="nominal", is_w_like=False):
    ...
    for key,helper in helper_stat.items():
        ...
        helper_helicity, helper_helicity_axes = wremnants.make_muon_eff_stat_helpers_helicity(helper)
        ...
        effStatTnP = df.HistoBoost(name, axes, [*cols, f"effStatTnP_{key}_ByHelicity_tensor"], tensor_axes = helper_helicity_axes)
        results.append(effStatTnP)
    
    df = df.Define("effSystTnP_weight", helper_syst, [*muon_columns_syst, "nominal_weight"])
    helper_syst_helicity, helper_syst_helicity_axes = wremnants.make_muon_eff_syst_helper_helicity(helper_syst)
...
def add_L1Prefire_unc_hists(results, df, helper_stat, helper_syst, axes, cols, base_name="nominal"):
    ...
    prefirebyhelicity_stat, prefire_axes_stat = wremnants.make_muon_prefiring_helper_stat_byHelicity(helper_stat)
    ...
    muonL1PrefireStat = df.HistoBoost(name, axes, [*cols, "muonL1PrefireStatByHelicity_tensor"], tensor_axes = prefire_axes_stat)
    ...
    prefirebyhelicity_syst, prefire_axes_syst = wremnants.make_muon_prefiring_helper_syst_byHelicity()
    ...
    muonL1PrefireSyst = df.HistoBoost(name, axes, [*cols, "muonL1PrefireSystByHelicity_tensor"], tensor_axes = prefire_axes_syst)
    ...
    ecalL1Prefire = df.HistoBoost(name, axes, [*cols, "ecalL1PrefireByHelicity_tensor"], tensor_axes = prefire_axes_syst)
